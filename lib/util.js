...
function isSimple(node){
	...
	if (n.MemberExpression.check(node)) {
		...
		if (isSimple(node.object)){
			...
			if (
				n.ObjectExpression.check(node.object) && !(node.property.name in Object)
			) return true;

			//doesn’t call string method
			if (
				n.Literal.check(node.object) && typeof node.object.value === 'string' && !(node.property.name in String)
			) return true;

			//doesn’t call number method
			if (
				n.Literal.check(node.object) && typeof node.object.value === 'number' && !(node.property.name in Number)
			) return true;

			//doesn’t call array method
			if (
				n.ObjectExpression.check(node.object) && !(node.property.name in Array)
			) return true;
		}
